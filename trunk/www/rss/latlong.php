...
function IsLatLong($str, &$latlong)
{
	...
	if (!$result)
	{
		if (preg_match('/(?<degreesLatitude>\d+)[°|º](\s*(?<minutesLatitude>\d+(.\d+)?)\')?((?<secondsLatitude>\d+)\'\')?\s*(?<hemisphereLatitude>[S|N])(;|,|–|\/)?\s*(?<degreesLongitude>\d+)°(\s*(?<minutesLongitude>\d+(.\d+)?)\')?((?<secondsLongitude>\d+)\'\')?\s*(?<hemisphereLongitude>[W|E])/', $str, $matches)) 
...
//	array_push($tests, 'N 0°6\'41"; W 77°22\'28"');
	...
	array_push($tests, "00º21'N, 034º51'E.");
