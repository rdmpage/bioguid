  ...
  */
	...
	// lazierLoad default options
		var lazierLoadDefaultOptions = {
			...
			treshold		: 300,							// Offset from bottom to start preloading
			extensions		: ['gif','png','jpg','jpeg'],	// Array of extensions to lazyLoad
			
			replaceImage	: gWebRoot + "/images/blank.gif",					// Placeholder image to show instead of the image (best leave unchanged to this blank.gif!)
			loadingImage	: gWebRoot +"/images/loading.gif",				// Loading indicator
	 ...
	 */
		...
		JS_BRAMUS.lazierLoadImage.prototype 	= {
			...
			initialize			: function(image, options) {
				...
				//alert(this.position[1] + ' ' + this.viewportHeight);
				...
				// image not "above the fold"
					...
					// rdmp
					Event.observe($('thumbnail_list'), 'scroll', this.lazyScroller.bind(this), false);
				}
			},
...
//					if ((document.viewport.getScrollOffsets()[1] + document.viewport.getHeight() + parseInt(this.options.treshold)) > this.position[1]) {
					...
					if ((Math.abs($('thumbnail_list').viewportOffset()[1] - $('thumbnail_list').positionedOffset()[1]) + $('thumbnail_list').getHeight() + parseInt(this.options.treshold)) 
						...
						// rdmp Stop observing container scroller
						Event.stopObserving($('thumbnail_list'), 'scroll', this.lazyScroller);
